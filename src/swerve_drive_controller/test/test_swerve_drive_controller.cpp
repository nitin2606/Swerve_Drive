...
void SwerveDriveControllerTest::SetUpController() {
  ...
  auto init_result = controller_->init("swerve_controller_test",  // controller_name
                                       "",                        // urdf (empty for test)
...
void SwerveDriveControllerTest::SetUpInterfaces() {
  ...
  controller_->assign_interfaces(std::move(command_interface_handles_),
...
TEST_F(SwerveDriveControllerTest, test_on_deactivate_success) {
  ...
  ASSERT_EQ(controller_->on_activate(rclcpp_lifecycle::State()),
            controller_interface::CallbackReturn::SUCCESS);
  EXPECT_EQ(controller_->on_deactivate(rclcpp_lifecycle::State()),
            controller_interface::CallbackReturn::SUCCESS);
}
...
TEST_F(SwerveDriveControllerTest, test_on_cleanup_success) {
  ...
  ASSERT_EQ(controller_->on_configure(rclcpp_lifecycle::State()),
            controller_interface::CallbackReturn::SUCCESS);
  EXPECT_EQ(controller_->on_cleanup(rclcpp_lifecycle::State()),
            controller_interface::CallbackReturn::SUCCESS);
}
...
TEST_F(SwerveDriveControllerTest, test_on_error_success) {
  ...
  ASSERT_EQ(controller_->on_configure(rclcpp_lifecycle::State()),
            controller_interface::CallbackReturn::SUCCESS);
  EXPECT_EQ(controller_->on_error(rclcpp_lifecycle::State()),
            controller_interface::CallbackReturn::SUCCESS);
}
...
TEST_F(SwerveDriveControllerTest, test_on_shutdown_success) {
  ...
  EXPECT_EQ(controller_->on_shutdown(rclcpp_lifecycle::State()),
            controller_interface::CallbackReturn::SUCCESS);
}
...
TEST_F(SwerveDriveControllerTest, test_update_inactive_state) {
  ...
  ASSERT_EQ(controller_->on_deactivate(rclcpp_lifecycle::State()),
            controller_interface::CallbackReturn::SUCCESS);
  EXPECT_EQ(controller_->update(node_->now(), rclcpp::Duration::from_seconds(0.02)),
            controller_interface::return_type::OK);
  for (size_t i = 0; i < 4; ++i) {  // Wheel velocities
    EXPECT_EQ(command_values_[i], 0.0) << "Wheel velocity " << i << " not zeroed";
  }
  for (size_t i = 4; i < 8; ++i) {  // Axle positions
...
TEST_F(SwerveDriveControllerTest, test_update_with_velocity_command) {
  ...
  // Update controller
  auto time = node_->now();
  EXPECT_EQ(controller_->update(time, rclcpp::Duration::from_seconds(0.02)),
            controller_interface::return_type::OK);
