...
controller_interface::return_type SwerveController::update(const rclcpp::Time &time,
  ...
  if (open_loop_) {
    std::array<double, 4> velocity_array = {front_left_velocity, front_right_velocity,
                                            rear_left_velocity, rear_right_velocity};
    std::array<double, 4> steering_angles = {front_left_angle, front_right_angle, rear_left_angle,
                                             rear_right_angle};
    odometry_ = swerveDriveKinematics_.update_odometry(velocity_array, steering_angles,
                                                       update_dt.seconds());
  } else {
    ...
    std::array<double, 4> steering_angles = {front_left_wheel_angle, front_right_wheel_angle,
                                             rear_left_wheel_angle, rear_right_wheel_angle};

    odometry_ = swerveDriveKinematics_.update_odometry(velocity_feedback_array, steering_angles,
                                                       update_dt.seconds());
  }
...
std::shared_ptr<Wheel> SwerveController::get_wheel(const std::string &wheel_name) {
  ...
  if (command_handle == command_interfaces_.end()) {
    RCLCPP_ERROR(get_node()->get_logger(), "Unable to obtain joint command handle for %s",
                 wheel_name.c_str());
    return nullptr;
  }

  const auto state_handle = std::find_if(
      state_interfaces_.begin(), state_interfaces_.end(), [&wheel_name](const auto &interface) {
        return interface.get_name() == (wheel_name + "/velocity") &&
               interface.get_interface_name() == HW_IF_VELOCITY;
      });
...
std::shared_ptr<Axle> SwerveController::get_axle(const std::string &axle_name) {
  ...
  const auto command_handle = std::find_if(
      command_interfaces_.begin(), command_interfaces_.end(), [&axle_name](const auto &interface) {
  ...
  const auto state_handle = std::find_if(
      state_interfaces_.begin(), state_interfaces_.end(), [&axle_name](const auto &interface) {
        return interface.get_name() == (axle_name + "/position") &&
               interface.get_interface_name() == HW_IF_POSITION;
      });
