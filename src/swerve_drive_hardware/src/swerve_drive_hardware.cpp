...
hardware_interface::CallbackReturn SwerveDriveHardware::on_init(const hardware_interface::HardwareInfo & info)
{
  ...
  logger_ = std::make_shared<rclcpp::Logger>(
    rclcpp::get_logger("controller_manager.resource_manager.hardware_component.system.SwerveDriveBot")
  );
  ...
  command_steering_angles_.resize(info_.joints.size()/2, std::numeric_limits<double>::quiet_NaN());
...
std::vector<hardware_interface::CommandInterface> SwerveDriveHardware::export_command_interfaces(){
    ...
    for (auto i = 0u; i < info_.joints.size(); i++){
      ...
      RCLCPP_INFO(rclcpp::get_logger("SwerveDriveHardware"), "JOINT NAME %s", joint.name.c_str());
      // RCLCPP_INFO(rclcpp::get_logger("SWERVE DRIVE HARDWARE"), "COMMAND_INTERFACE %s", joint.command_interfaces[1].name.c_str());

      if (joint.command_interfaces[0].name == hardware_interface::HW_IF_VELOCITY) {
        // RCLCPP_INFO(rclcpp::get_logger("SwerveDriveHardware"), "Added Velocity Joint: %s", joint.name.c_str() );
        ...
        // RCLCPP_INFO(rclcpp::get_logger("SwerveDriveHardware"), "Added Position Joint: %s", joint.name.c_str() );
        command_interfaces.emplace_back(hardware_interface::CommandInterface(
          joint.name, hardware_interface::HW_IF_POSITION, &command_steering_angles_[counter_steering_angles]));
    ...
    RCLCPP_INFO(rclcpp::get_logger("SwerveDriveHardware"), "[SWERVE_DRIVE_HARDWARE] Length Of Command Interfaces: %ld", command_interfaces.size());

    return command_interfaces;
}


hardware_interface::CallbackReturn SwerveDriveHardware::on_configure(const rclcpp_lifecycle::State & /*previous_state*/)
{
...
hardware_interface::CallbackReturn SwerveDriveHardware::on_activate(const rclcpp_lifecycle::State & /*previous_state*/)
{
...
hardware_interface::CallbackReturn SwerveDriveHardware::on_deactivate(const rclcpp_lifecycle::State & /*previous_state*/){
  RCLCPP_INFO(rclcpp::get_logger("SwerveDriveHardware"), "Deactivating ...please wait...");
  RCLCPP_INFO(rclcpp::get_logger("SwerveDriveHardware"), "Successfully deactivated!");
  return hardware_interface::CallbackReturn::SUCCESS;
}

hardware_interface::CallbackReturn SwerveDriveHardware::on_cleanup(const rclcpp_lifecycle::State & /*previous_state*/){
  RCLCPP_INFO(rclcpp::get_logger("SwerveDriveHardware"), "CLEANING UP HARDWARE ...");
  return hardware_interface::CallbackReturn::SUCCESS;
}

hardware_interface::CallbackReturn SwerveDriveHardware::on_shutdown(const rclcpp_lifecycle::State & /*previous_state*/){
  RCLCPP_INFO(rclcpp::get_logger("SwerveDriveHardware"), "SHUTTING DOWN HARDWARE ...");
  return hardware_interface::CallbackReturn::SUCCESS;
}

hardware_interface::CallbackReturn SwerveDriveHardware::on_error(const rclcpp_lifecycle::State & /*previous_state*/){
  RCLCPP_ERROR(rclcpp::get_logger("SwerveDriveHardware"), "ERROR STATE REACHED !!!");
  return hardware_interface::CallbackReturn::ERROR;
}


hardware_interface::return_type swerve_drive_hardware::SwerveDriveHardware::read(const rclcpp::Time& time, const rclcpp::Duration& period){
...
hardware_interface::return_type swerve_drive_hardware::SwerveDriveHardware::write(const rclcpp::Time& time, const rclcpp::Duration& period){
...
PLUGINLIB_EXPORT_CLASS(swerve_drive_hardware::SwerveDriveHardware, hardware_interface::SystemInterface)
